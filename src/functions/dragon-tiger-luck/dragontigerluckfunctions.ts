...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>("UPDATE dragontigerluckjson SET JSON = ? WHERE id = ?", [json, id])
      return res[0]
   },
   async getjsondragontigerlucky(id: number) {
      const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM dragontigerluckjson WHERE id=?", [id])
      return res[0]
   },
   async createjsondragontigerlucky(id: number) {
      const res = await promisePool.query<ResultSetHeader>("INSERT INTO dragontigerluckjson (id) VALUES (?)", [id])
