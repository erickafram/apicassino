...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>("UPDATE ganeshagoldjson SET JSON = ? WHERE id = ?", [json, id])
      return res[0]
   },
   async getjsonganesha(id: number) {
      const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM ganeshagoldjson WHERE id=?", [id])
      return res[0]
   },
   async createjsonganesha(id: number) {
      const res = await promisePool.query<ResultSetHeader>("INSERT INTO ganeshagoldjson (id) VALUES (?)", [id])
