...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>("UPDATE jungledelightjson SET JSON = ? WHERE id = ?", [json, id])
      return res[0]
   },
   async getjsonjungle(id: number) {
      const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM jungledelightjson WHERE id=?", [id])
      return res[0]
   },
   async creajsonjungle(id: number) {
      const res = await promisePool.query<ResultSetHeader>("INSERT INTO jungledelightjson (id) VALUES (?)", [id])
