...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>("UPDATE fortunedragonplayerjson SET JSON = ? WHERE id = ?", [json, id])
      return res[0]
   },
   async getjsondragon(id: number) {
      const res = await promisePool.query<RowDataPacket[]>("SELECT * FROM fortunedragonplayerjson WHERE id=?", [id])
      return res[0]
   },
   async createjsondragon(id: number) {
      const res = await promisePool.query<ResultSetHeader>("INSERT INTO fortunedragonplayerjson (id) VALUES (?)", [id])
