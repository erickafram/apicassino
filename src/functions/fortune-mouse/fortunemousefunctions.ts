...
export default {
   ...
   async savejsonspin(id: number, json: any) {
      const res = await promisePool.query<ResultSetHeader>(
         "UPDATE fortunemouseplayerjson SET JSON = ? WHERE id = ?",
         [json, id],
      )
      return res[0]
   },
   async getjsonmouse(id: number) {
      const res = await promisePool.query<RowDataPacket[]>(
         "SELECT * FROM fortunemouseplayerjson WHERE id=?",
         [id],
      )
      return res[0]
   },
   async createjsonmouse(id: number) {
      const res = await promisePool.query<ResultSetHeader>(
         "INSERT INTO fortunemouseplayerjson (id) VALUES (?)",
         [id],
      )
